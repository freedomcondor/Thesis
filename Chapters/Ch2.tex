...
Such methods can offer scalability and robustness due to their decentralized interactions
    \cite{santos2019decentralized,
          ...
          siligardi2019robust}.
%However, relying solely on local interactions can cause the system to become trapped in locally optimal states, limiting the coverage quality.
...
The motion control in these is approaches is fully decentralized, but sometimes centralized coordination is used for later steps, such as fusing collected information.
...
In addition, switching formations have been explored: that is, a series of target formations are predefined and the system is triggered to switch between them %by updating inter-agent constraints while preserving overall formation rigidity
    \cite{desai1999control,
          desai2002modeling}.
...
%From a broader robot system perspective, 
A fully autonomous multi-robot system should %not only execute individual tasks but also operate autonomously, 
be able to make decisions and adapt to a changing environment without requiring human intervention in order to accomplish its goals. 
...
This goal is particularly challenging for swarm systems, since robots operate with only local information,
%hindering consensus in decision making, 
which often leads to long convergence times and makes collective behaviors difficult to design,
and the lack of centralized control makes it difficult for human commands to propagate through the swarm.
...
Autonomy is often regarded as a central long-term goal of robotic systems—the ability for robots to operate without 
requiring human supervision or intervention.
...
Classical collective decision-making problems include the best-of-N problem, of which the binary (best-of-two) case is a special instance
...
Despite these progress, decision-making in swarms generally remains an isolated task
%a simple reactive task for environmental analysis,
rather than being integrated into a broader understanding of the mission context.
...
Some existing research explore interfaces to manipulate the swarm such as gesture control
...
and also make the design of effective local behaviors challenging.
...
%When supervising the swarm, one requirement is that the programmer needs to be able to re-program the swarm easily and online.
...
Classically,
programs are pre-designed and downloaded offline to all the robots before deployment, for example,
    \cite{rubenstein2014programmable,
          valentini2016collective,
          werfel2014designing,
          dorigo2013swarmanoid}.
...
Notably, automatic design methods demonstrate how optimization and evolutionary algorithms can produce robust swarm behaviors without manual tuning
    \cite{francesca2014automode,
          francesca2016automatic,
          birattari2019automatic,
          salman2024automatic}.
...
However, studies in over-the-air program updating for robotic swarms also highlights fundamental limitations such as slow propagation resulting from consensus mechanisms
    \cite{de2009energy,
...
Swarm systems shows some inherent fault tolerance property because of their redundancy and absence of single points of failure
...
It can appear, disappear, and reappear and then recovers.
...
Overall, although decentralized swarm systems benefit from inherent fault tolerance,
...
%In summary, centralized approaches offer global optimality and ease of control,
