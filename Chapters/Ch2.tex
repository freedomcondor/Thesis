...
These systems seek to bypass the micro-macro problem,
                      ...
                      and improve the performance of a decentralized system,
...
and discusses aspects of swarm manageability, including human-swarm iteraction, swarm re-programming, and swarm autonomy.
...
In addition, hierarchical approaches impose an additional topological organization on the swarm.
...
In this way, information flows in a more structured manner in the swarm.
Many tasks benefit from this, such as flocking
...
and thereby ease the design of the behavior and reduce task completion time.
...
No existing approach has provided a general mechanism for self-organized hierarchy—that is, adaptively forming and maintaining hierarchical structures without predefined topology or fixed leadership.
...
Purely decentralized swarms are highly scalable and fault tolerant,
but often difficult to design
and may exhibit suboptimal global coordination,
                lower solution quality,
                and slower task completion in certain tasks,
especially when global information is required.
Centralized systems, by contrast, are easy to design, achieve higher task optimality through global coordination, but lose scalability and fault tolerance.
...
but role specification centralized in practice.
These approaches focus on achieving stable rigid-body formations with each robot referencing its neighbors using distance-based constraints
...
Overall, these methods assumes fixed roles and full knowledge of the desired formation, emphasizing control-theoretic guarantees over flexibility or adaptivity, 
but offering little support for dynamic or fault-tolerant operation and lack flexibility after deployment.

\subsubsection{Switching Roles}

\textcolor{blue}{Why role switching is important}

A key feature of swarm robotics is the ability to deploy robots without pre-assigned roles.
...
While formation focuses on maintaining morphology of the swarm, coverage addresses a different aspect of swarm coordination.
...
They are easy to implement and require only minimal information but often suffer from long finish time and redundant traversal
...
These strategies often require predefined roles, global map knowledge, or explicit task allocation, which introduces centralized components but yields fast and predictable coverage
...
While decentralized dispersion and exploration are robust and easy to scale, they often lack global optimality.
Conversely, sweep-based and centrally coordinated methods achieve highly fast and complete coverage but depend on predefined assignments or global knowledge, limiting their adaptability in large, dynamic environments.

\subsection{Navigation Path Planning}

In recent applications of multi-robot systems, especially for drones systems, path planning is a key component.
...
these researches employ heuristics and approximate solutions to generate high-quality global trajectories for robots
...
It represents a fully centralized form of swarm coordination,
