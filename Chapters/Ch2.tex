...
% 2 Swarm vs. centralized approaches for different tasks (explain what is possible to do with both, for those tasks -- current limitations areswar implied)
...
Following these insights, the concept of self-organization refers to the principle that individuals interact locally with one another without centralized control, and complex global behavior emerges.
Swarm Robotics aims to leverage the principles of self-organization to construct decentralized robotic systems that are scalable, adaptive, and fault tolerant
    \cite{dorigo2014swarm,    % scholarpedia on swarm robotics
          dorigo2020reflections,Swarm intelligence often draws inspiration from collective behaviors in social animals such as ants, birds, and fish
...
Owing to the absence of centralized control, such systems can be deployed over large spatial domains and can scale up to vast numbers of robots without demanding increasing capabilities from each individual robot.
...
yet purely decentralized swarms are often difficult to design and manage, and may take a long time to complete the task, especially for tasks that require global information, such as environmental analysis and decision making.
    \cite{dorigo2021swarm,
...
Flat and homogeneous swarms, inspired by classical examples from nature,
...
In some works, certain individuals are assigned explicit leadership roles
    \cite{gu2009leader,
          amraii2014explicit}.
...
because the topological organization is usually predefined, these systems suffer from low scalability and single points of failure.
...
No existing approach has provided a general mechanism for self-organizing hierarchies—that is, adaptively forming and maintaining hierarchical structures without predefined topology or fixed leadership.
...
and they often exhibit inferior performance. %such as slower task completion times. %in certain tasks.
In contrast, systems with centralized or predetermined control are much easier to design.
They can also complete many tasks more quickly and with higher accuracy by exploiting global information, but lack scalability and fault tolerance.
%This section reviews these different control approaches and their advantages and disadvantages and across several key example tasks in multi-robot systems.
...
%Multi-robot path optimization usually employs heuristics such as genetic algorithm and particle swarm optimization %and approximate solutions to generate high-quality global trajectories for robots
Multi-robot path optimization usually employs heuristics such as genetic algorithms and particle swarm optimization %and approximate solutions to generate high-quality global trajectories for robots
...
%This approach enables agile navigation in cluttered environments, but the generated solutions are of course not perfectly optimal, in contrast to what could in principle be achieved using offline global optimization.  
...
In summary, centralized planners can use global information to generate high-quality paths 
                                 but scale poorly and lack robustness to failures, 
and existing distributed approaches to path generation still fall short of providing fully scalable and fault-tolerant mechanisms for both defining and reaching goal positions.
...
Given the emphasis of swarm robotics on scalability and fault tolerance, we focus on decentralized formation control, which is also the focus of the majority of existing work from the control theory literature.
%In addition, we also examine the multi robot formation from the angle of role assignment.
In addition, we also examine multi-robot formation from the perspective of role assignment.
...
%Existing swarm researches primarily focus on a single aspect of autonomy such as action, perception, and adaptation
...
%As a result, collective decision making play an important role in swarm autonomy.
That's why collective decision making is one of the most-studied aspects of swarm autonomy.
...
%Despite these progress, decision-making in swarms generally remains an isolated task rather than being integrated into a broader understanding of the mission context.
...
%Current research mainly focus on two sides : human interfaces and swarm influence.
Current research mainly focuses on two aspects:
the design of human–swarm interfaces,
and the mechanisms through which human commands applied to a small subset of robots can propagate to influence the collective behavior of the swarm
...
Among these works,
%, by default, 
%operators are able to communicate with all the robots in the swarm to send commands collectively
...
%For decentralized methods where human communicate with only one robot or subset of the swarm, researches focus on how to make the subset of the swarm influence the whole swarm
For decentralized methods where humans communicate with only one robot or a subset of the swarm, research focuses on how a subset of the swarm can influence the entire swarm
...
Automatic design methods demonstrate how optimization and evolutionary algorithms can produce robust swarm behaviors without manual tuning
    \cite{francesca2014automode,
          francesca2016automatic,
          birattari2019automatic,
          salman2024automatic}.
%They usually fall in this category, as they typically require long trial-and-error optimization before deployment.
These methods typically rely on extensive trial-and-error optimization prior to deployment and are therefore most often applied in an offline manner.
...
Swarm systems show some degree of inherent fault tolerance because of their redundancy and absence of single points of failure
    \cite{dorigo2014swarm,
          hamann2018swarm}.
%Most classical swarm tasks, such as coverage or collective decision making, can tolerate a subset of robots fail.
Most classical swarm tasks, such as coverage or collective decision making, can tolerate a subset of robots failing.
%Robots simply continue to operate as normal, without knowing or caring about the failure peers 
...
\textcolor{blue}{But fault tolerance is not completely solved}
...
%Yet purely decentralized swarms are often difficult to design and manage, and may take long time to finish the task, especially in tasks that require global information, such as environmental analyzing and decision making.
Yet purely decentralized swarms are often difficult to design and manage, and may take a long time to complete the task, especially in tasks that require global information, such as environmental analyzing and decision making.

%In summary, centralized approaches offer easy solution to design and manage the swarm and can often coordinate the robots to finish the task quickly, 
In summary, centralized approaches offer an easy solution for designing and managing the swarm and can often coordinate the robots to finish the task quickly,
while decentralized swarms provide robustness and scalability.
