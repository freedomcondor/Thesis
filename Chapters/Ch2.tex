...
\subsection{Navigation, Search and Rescue}

\textcolor{red}{only about coverage}
\textcolor{red}{path planning, goes to other places, maybe obstacle avoidance, or simply drop them. check what does Aryo say, and check their applications}
...
For navigation : offline centralized way for path planning.
    ...
    \cite{yu2016optimal}
    \cite{kushleyev2013towards} micro drones
online, but all information shares, not scalable
    \cite{zhou2022swarm}
...
\textcolor{red}{not about hierachy, doesn't hit the point}.

\textcolor{red}{Sinan :}
Swarm robotics usually studies passive tolerance to permanent faults—that is, faults such as electromechanical failures that will remain unless they are actively repaired.
When relying on passive fault tolerance,
studies have usually demonstrated that a swarm continues its mission after some or many robots have failed,
either by continuing with fewer robots
    \cite{bjerknes2013fault}
    \cite{rubenstein2014programmable}
    \cite{khadidos2015exogenous}
or by replacing/repairing the failed robots without pausing the mission
