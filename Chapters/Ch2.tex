...
Such methods offer strong scalability and robustness
...
These strategies are self-organized.
...
There are also mixed approaches. For example, \cite{scherer2015autonomous} uses motion trajectories that are predetermined but manages the collection of environmental information in a self-organized manner.

\textcolor{blue}{summarize: a gap}

To summarize, 
%coverage tasks clearly show the dichotomy between self-organization and centralization.
while random exploration is robust and scalable, it often lacks global optimality.
...
However, the target position of each robot is pre-defined and fixed.
...
Most self-organized formation controllers rely on relative localization, using distance sensors, ultraviolet markers, or fiducial markers
...
Most existing methods assume that each robot is preassigned a fixed role in the formation.
During deployment,
...
%making motion control,
%but role specification predetermined in practice.
